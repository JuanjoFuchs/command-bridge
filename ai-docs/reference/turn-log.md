...
applies_to: command_bridge/store.py, command_bridge/cli.py (watch)
read_before: changing the turn schema, watch semantics, or anything that appends
---

# Turn log and cursor contract

One JSONL file per session at `<COMMAND_BRIDGE_DIR>/<session>.jsonl`, one turn per line. The server appends;

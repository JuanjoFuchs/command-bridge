...
applies_to: command_bridge/security.py, command_bridge/server.py
...
1. **Network layer — who can reach the socket.** An IP allowlist over CIDRs. Loopback always
   allowed; anything else opt-in via `COMMAND_BRIDGE_ALLOW_CIDRS`. For the phone that means the Tailscale
...
itself inside `COMMAND_BRIDGE_TRUSTED_PROXIES`, which defaults to empty — meaning the header is ignored

...
> Run `command-bridge describe` and follow it end to end: install anything missing, start command-bridge
...
The manual path, if you would rather not hand the whole thing to an agent. Start command-bridge with
...
rather than guessing, because a bridge that stops whenever the room makes a noise is worse than
...
| `specs/` | 001 CLI identity · 002 packaging · 003 npm · 004 turn detection |

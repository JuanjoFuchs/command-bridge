#!/usr/bin/env python
"""The one entry point every `command-bridge` shim executes. Not meant to be typed by a human.

WHY THIS FILE EXISTS. The shims used to invoke the CLI as an inline program:

    python -c "import sys; sys.path.insert(0, r'$COMMAND_BRIDGE_ROOT'); from command_bridge.cli import main; ..."

which fails in two ways that are only visible on Windows. First, `$COMMAND_BRIDGE_ROOT` had to be spliced
...
# Put the repo root FIRST so `command-bridge` resolves to this checkout even if another one is installed

...
a new shell to pick up. In every one of those `python -m command_bridge` works immediately, which
...
`python -m command_bridge` has the package importable by definition, and the frozen bundle has it
collected.
"""
from command_bridge.cli import main

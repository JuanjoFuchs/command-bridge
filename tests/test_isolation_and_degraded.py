...
def clean_env(monkeypatch):
    for var in ("COMMAND_BRIDGE_HOME", "COMMAND_BRIDGE_DIR", "COMMAND_BRIDGE_MODELS_DIR",
                "COMMAND_BRIDGE_ENV_FILE"):
...
def test_home_scopes_all_three_paths(clean_env, tmp_path):
    ...
    clean_env.setenv("COMMAND_BRIDGE_HOME", str(tmp_path))
    for path in (config.session_dir(), config.models_dir(), config.env_file_path()):
        assert str(tmp_path) in path, f"{path} escaped COMMAND_BRIDGE_HOME"
...
def test_the_session_variable_alone_does_not_isolate_settings(clean_env, tmp_path):
    ...
    `COMMAND_BRIDGE_DIR` scopes turn logs and nothing else. That is legitimate behaviour — it is
    ...
    """
    clean_env.setenv("COMMAND_BRIDGE_DIR", str(tmp_path / "sessions"))
    assert str(tmp_path) in config.session_dir()
    assert str(tmp_path) not in config.env_file_path(), (
        "if this ever starts passing, COMMAND_BRIDGE_DIR has grown scope and the docs must follow"
    )
...
def test_an_explicit_variable_still_beats_home(clean_env, tmp_path):
    ...
    clean_env.setenv("COMMAND_BRIDGE_HOME", str(tmp_path / "home"))
    clean_env.setenv("COMMAND_BRIDGE_MODELS_DIR", str(tmp_path / "shared-models"))
...
def test_doctor_names_the_paths_other_installs_also_use(capsys, tmp_sessions, clean_env):
    ...
    assert "isolate_with" in runtime and "COMMAND_BRIDGE_HOME" in runtime["isolate_with"]
...
def test_spawning_piper_is_reported_as_degraded(capsys, tmp_sessions, monkeypatch):
    ...
    monkeypatch.setenv("COMMAND_BRIDGE_PIPER_INPROCESS", "0")
    from tests.test_cli_surface import run
